...
| --- | --- | --- |
| `SkyCommandHandlers` in `domain` is `@Component` | Axon 5 Entity Model requires external command handlers to be Spring beans so the Axon-Spring bridge can wire them with `@InjectEntity` + `EventAppender`. Co-located in `domain` to keep the bounded context coherent. | Don't move it out of `domain`. `@Component` is the one Spring stereotype tolerated in `domain` for this reason; keep `@Service` / `@Configuration` / `@Repository` out of `domain` (the module declares Spring `compileOnly`, and the inward-only dependency direction is enforced by the Gradle module graph). |
