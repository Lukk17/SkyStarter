...
- [.mcp.json.example](../.mcp.json): Claude Code project scope. Schema key `mcpServers`. Env-var syntax:
  `${VAR}` and `${VAR:-default}`.
- [opencode.json.example](../opencode.json): OpenCode plus Kilo Code project scope (they share the file).
...
in [.mcp.json.example](../.mcp.json) or as hardcoded literals in [opencode.json.example](../opencode.json).
...
- [.mcp.json.example](../.mcp.json) (Claude Code) uses `${VAR:-default}`, so every env var has a resolvable
  fallback. Files parse with no env set.
- [opencode.json.example](../opencode.json) (OpenCode plus Kilo) uses `{env:VAR}` for tokens and **hardcodes
...
Edit [.mcp.json.example](../.mcp.json) and [opencode.json.example](../opencode.json) in the
